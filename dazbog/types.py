...
from dazbog.tools import camel_to_snake
...
def generate_types(root: DocumentNode) -> t.Iterator[str]:
    ...
    yield "# generated by dazbog, please do not change manually"
    definitions = list(
        filter(
            None,
            (
    ...
    yield ""
    for definition in sorted(definitions, key=operator.attrgetter("weight")):
        if definition:
...
@singledispatch
def has_arguments(
    field: t.Union[ast.FieldDefinitionNode, ast.InputValueDefinitionNode]
) -> bool:
    raise RuntimeError()
...
@has_arguments.register
def input_has_arguments(field: ast.InputValueDefinitionNode) -> bool:
    return False
...
@type_node.register
def named_type_mode(
    named_type_node: ast.NamedTypeNode, defined_types: t.Set[str], optional=optional
) -> PythonType:
